...
const ApplicationRequestPage = () => {
  ...
  useEffect(() => {
    ...
    if (isDefaultCounterparty) {
      ...
      constructionSiteService
        .getCounterpartyObjects(user?.counterpartyId)
        .then((response) => {
        ...
        .catch((error) => {
          console.error(
            "Error loading counterparty construction sites:",
            error,
          );
          setAvailableSites([]);
        })
      ...
      counterpartyService
        ...
        .then((response) => {
          const rawCounterparties =
            ...
            [];
          const counterparties = Array.isArray(rawCounterparties)
            ? rawCounterparties
            : [];
  ...
  const activeColumns = selectedColumns.filter(
    (col) => col.enabled && col.key !== "number",
  );
  ...
  const handleCreateRequest = async () => {
    ...
    try {
      ...
      const employeesToExport = availableEmployees.filter((emp) =>
  ...
  return (
    ...
    >
                ...
                {availableEmployees.map((employee) => (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: "4px",
                      background: selectedEmployees.includes(employee.id)
                        ? "#f6f8fb"
                        : "#fff",
                      cursor: "pointer",
                      transition: "all 0.3s",
      ...
      >
        <Button onClick={() => navigate("/employees")} style={{ flex: 1 }}>
          Отмена
        </Button>
        <Button
          type="primary"
          ...
          loading={isLoading}
          disabled={
            selectedEmployees.length === 0 || availableEmployees.length === 0
          }
