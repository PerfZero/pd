...
const SecurityModal = ({ visible, onCancel, onSuccess }) => {
    ...
    try {
      ...
      if (selectedCounterparty) {
        filtered = filtered.filter((emp) =>
          emp.employeeCounterpartyMappings?.some(
            (m) => m.counterpartyId === selectedCounterparty,
          ),
        );
      }
      ...
      if (statusFilters.length > 0) {
        filtered = filtered.filter((emp) => {
          return statusFilters.some((filter) => {
            const statusMapping = emp.statusMappings?.find(
              (m) => m.statusGroup === "status" || m.status_group === "status",
            );
            const secureMapping = emp.statusMappings?.find(
              (m) =>
            ...
            if (filter === "tb_passed") {
              return (
                statusMapping?.status?.name === "status_tb_passed" ||
                statusMapping?.status?.name === "status_processed"
              );
            }
            ...
            if (filter === "not_blocked") {
              return (
                secureMapping?.status?.name === "status_secure_allow" ||
                !secureMapping
              );
            }
            if (filter === "blocked") {
              return (
                secureMapping?.status?.name === "status_secure_block" ||
                secureMapping?.status?.name === "status_secure_block_compl"
              );
            }
            return false;
          });
  ...
  }, [message, searchText, selectedCounterparty, statusFilters]);
  ...
  const columns = [
    ...
    },
    {
      ...
      align: "center",
      render: (_, record) => {
        const secureMapping = record.statusMappings?.find(
          (m) =>
            m.statusGroup === "status_secure" ||
            m.status_group === "status_secure",
        );
        ...
        const isBlocked =
          secureName === "status_secure_block" ||
          secureName === "status_secure_block_compl";
