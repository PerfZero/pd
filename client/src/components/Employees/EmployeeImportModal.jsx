...
const EmployeeImportModal = ({ visible, onCancel, onSuccess }) => {
        ...
        const mappedData = rawData.map((row, idx) => {
          ...
          const parseDate = (value) => {
            ...
            if (typeof value === "number") {
              ...
              return dayjs(new Date(date.y, date.m - 1, date.d)).format(
                "YYYY-MM-DD",
              );
            }
          ...
          const mapped = {
            counterpartyInn: normalize(
              row["ИНН организации"] || row["inn_organization"],
            ),
            counterpartyKpp: normalize(
            ...
            kig: normalize(kig),
            kigEndDate: parseDate(
              row["Срок окончания КИГ"] || row["kig_end_date"],
            ),
          ...
          if (idx < 3) {
            ...
            console.log(
              `  - ФИО: "${mapped.lastName}" "${mapped.firstName}" "${mapped.middleName}"`,
            );
  ...
  const handleValidate = async () => {
    ...
    try {
      setLoading(true);
      console.log(
        "📤 Отправляем данные на валидацию, записей:",
        fileData.length,
      );
  ...
  const handleConflictResolution = async () => {
    try {
      ...
      validationResult?.conflictingInns?.forEach((conflict) => {
        if (conflictResolutions[conflict.inn] === "update") {
          ...
          if (originalData) {
            ...
            console.log(
              "✅ Добавили counterparty данные к сотруднику:",
              employeeWithCounterparty,
            );
      ...
      console.log("📤 Отправляем для импорта:", {
        ...
        conflictsToUpdate: Object.values(conflictResolutions).filter(
          (r) => r === "update",
        ).length,
      });
    ...
    <div style={{ padding: "40px 20px" }}>
      ...
      <div style={{ marginBottom: "24px" }}>
        ...
        >
          ...
          <div>
            Организация, <strong>ИНН организации</strong>,{" "}
      ...
      >
        <div style={{ marginBottom: "8px" }}>
          <strong>ℹ️ Примечание:</strong> Столбец № пропускается. Столбцы, не
          указанные выше, игнорируются.
        </div>
        <div>
          <strong>🏢 Контрагенты:</strong> <strong>ИНН организации</strong> и{" "}
          <strong>КПП организации</strong> - контрагент должен быть вашей
          организацией или вашим субподрядчиком.
        </div>
      </div>
    </div>
  );
  ...
  const renderStep1 = () => (
    <div>
      ...
      {fileData && fileData.length > 0 ? (
        <Table
          dataSource={(fileData || []).map((item, idx) => ({
            ...item,
            _key: idx,
          }))}
          columns={[
                ...
                },
                {
                  ...
                  render: (_, record) => (
                    <Radio.Group
                      ...
                      onChange={(e) =>
                        handleConflictRadioChange(record.inn, e.target.value)
                      }
                    >
      ...
      <div>
        ...
        >
          ...
          >
            ...
            >
              <div
                style={{ fontSize: 32, fontWeight: "bold", color: "#faad14" }}
              >
                {importResult?.updated || 0}
              </div>
              <div style={{ color: "#666", fontSize: 14, marginTop: "4px" }}>
          ...
          {totalProcessed > 0 && (
            ...
            >
              <CheckCircleOutlined /> Всего обработано:{" "}
        ...
        {hasErrors && (
          ...
          >
            ...
            <div style={{ color: "#8c8c8c", fontSize: 12, marginBottom: 12 }}>
              Сотрудники успешно созданы, но возникли проблемы при
