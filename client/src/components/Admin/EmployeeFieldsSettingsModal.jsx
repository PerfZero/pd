...
import {
  ...
  FIELD_GROUPS,
  DEFAULT_FORM_CONFIG,
} from "../../shared/config/employeeFields";
...
const EmployeeFieldsSettingsModal = ({ visible, onCancel }) => {
  ...
  const handleChange = (type, key, property, value) => {
    const setConfig =
      type === "default" ? setLocalConfigDefault : setLocalConfigExternal;
    ...
    try {
      ...
      await Promise.all([
        settingsService.updateSetting(
          ...
          JSON.stringify(localConfigDefault),
        ),
        settingsService.updateSetting(
          "employee_form_config_external",
          JSON.stringify(localConfigExternal),
        ),
      ]);
  ...
  const columns = (type) => [
    ...
    },
    {
      ...
      render: (_, record) => {
        const config =
          type === "default" ? localConfigDefault : localConfigExternal;
        ...
        return (
          <Checkbox
            checked={isVisible}
            onChange={(e) =>
              handleChange(type, record.key, "visible", e.target.checked)
            }
          />
        );
      },
    },
    {
      ...
      render: (_, record) => {
        ...
        const isVisible = config[record.key]?.visible ?? record.defaultVisible;
        const isRequired =
          config[record.key]?.required ?? record.defaultRequired;
        ...
        return (
          <Checkbox
            ...
            onChange={(e) =>
              handleChange(type, record.key, "required", e.target.checked)
            }
          />
        );
      },
