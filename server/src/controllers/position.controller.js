...
export const createPosition = async (req, res, next) => {
  try {
    ...
    if (
      ...
      return next(
        new AppError(
          "Недостаточно прав. Добавлять должности могут только пользователи контрагента по умолчанию.",
          403,
        ),
      );
    }
...
export const updatePosition = async (req, res, next) => {
  try {
    ...
    if (
      ...
      return next(
        new AppError(
          "Недостаточно прав. Редактировать должности могут только пользователи контрагента по умолчанию.",
          403,
        ),
      );
    }
...
export const deletePosition = async (req, res, next) => {
  try {
    ...
    if (
      ...
      return next(
        new AppError(
          "Недостаточно прав. Удалять должности могут только пользователи контрагента по умолчанию.",
          403,
        ),
      );
    }
...
export const importPositions = async (req, res, next) => {
  try {
    ...
    if (
      ...
      return next(
        new AppError(
          "Недостаточно прав. Импортировать должности могут только пользователи контрагента по умолчанию.",
          403,
        ),
      );
    }
    ...
    console.log(
      `📦 Импорт должностей: всего ${validPositions.length}, батчей: ${batches.length}`,
    );
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      try {
        ...
        console.log(
          `✅ Батч ${batchIndex + 1}/${batches.length}: обработано ${result.length} записей`,
        );
      } catch (error) {
